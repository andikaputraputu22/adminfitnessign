<section id="pricing" class="pricing section dark-background">
  <div class="container text-center" data-aos="fade-up">
    <h2 class="fitnessign-text-green mb-5">
      Start building muscles today with our new Packages
    </h2>

    <div class="row justify-content-center">
      <!-- Private Coach -->
      <div class="col-md-4 mb-4">
        <div class="pricing-card bg-white text-dark p-4 rounded-4 shadow-sm h-100 d-flex flex-column justify-content-between">
          <div>
            <h4 class="fw-bold text-uppercase">PRIVATE COACH</h4>
            <p class="mb-3">Start Rp.120.000</p>
            <ul class="list-unstyled text-start small">
              <li>1 x session duration 60 minutes</li>
              <li>full payment or deposite system</li>
              <li>there is no expired period for session purchases</li>
              <li>Body condition assesment & fitness target consultation</li>
            </ul>
          </div>
          <a href="#" class="fitnessign-btn-get-started mt-3">Buy Now</a>
        </div>
      </div>

      <!-- Instructor Class -->
      <div class="col-md-4 mb-4">
        <div class="pricing-card bg-white text-dark p-4 rounded-4 shadow-sm h-100 d-flex flex-column justify-content-between">
          <div>
            <h4 class="fw-bold text-uppercase">INSTRUCTOR CLASS</h4>
            <p class="mb-3">Start Rp.750.000</p>
            <ul class="list-unstyled text-start small">
              <li>Duration 90 minutes</li>
              <li>full payment or deposite system</li>
              <li>Maximum participants 20 trainee</li>
              <li>Suitable for community or office. Equipment is provided by our coaches</li>
            </ul>
          </div>
          <a href="#" class="fitnessign-btn-get-started mt-3">Buy Now</a>
        </div>
      </div>
    </div>
  </div>
</section>
